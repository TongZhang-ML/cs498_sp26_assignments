...
\begin{enumerate}
\item \noindent\textbf{A Simple baseline CNN architecture.}
...
\item  (6 points) Implement the baseline CNN model
\texttt{MNISTConvNet}.
...
\begin{enumerate}
\item  (8 points) Implement the residual block 
\texttt{ResBlock} and shallow ResNet \texttt{MNISTResNet}  as described above.
\item  (3 points) \textbf{(written)}
  Consider an input feature map of shape $(C,H,W)$.  
When using \texttt{Conv2d(C, C, kernel\_size=3, stride=1, padding=1)},  
the output feature map has the same shape $(C,H,W)$.

\medskip
\noindent
Answer the following questions:
\begin{enumerate}
\item What is the output feature map shape when using
\texttt{Conv2d(C, C, kernel\_size=3, stride=1, padding=0)}?
\item What is the output feature map shape when using
\texttt{Conv2d(C, C, kernel\_size=1, stride=2, padding=0)}?
\item What padding size $p$ should be used so that
\texttt{Conv2d(C, C, kernel\_size=5, stride=1, padding=$p$)}
produces an output feature map of shape $(C,H,W)$?
\end{enumerate}
\begin{solution}
\noindent
1. \texttt{kernel\_size=3}, \texttt{stride=1}, \texttt{padding=0}:
output shape $(C, H-2, W-2)$.

\medskip
\noindent
2. \texttt{kernel\_size=1}, \texttt{stride=2}, \texttt{padding=0}:
output shape
\[
\left(C, \left\lfloor \frac{H+1}{2} \right\rfloor,
           \left\lfloor \frac{W+1}{2} \right\rfloor \right).
\]

\medskip
\noindent
3. To keep the output shape $(C,H,W)$ with \texttt{kernel\_size=5} and
\texttt{stride=1}, the required padding is $p=2$.
\end{solution}
...
Run the following experiments on MNIST:
  Use \texttt{CrossEntropyLoss}. Train for 5 epochs (to keep runtime short).

Report validation accuracy for each run, and include a plot of epoch vs (train loss, validation loss) for each run.
Your runs should achieve \textbf{at least 98\%} validation accuracy.
\begin{enumerate}
    \item (5 points) Implement the function fit\_and\_evaluate according to the provided docstring specifications. 

    \item (2 points) \textbf{(written)} Baseline CNN with \texttt{use\_bn=True}, trained
      with SGD.
      \begin{solution}
        ...
        \end{center}
      \end{solution}
    \item (2 points) \textbf{(written)} Baseline CNN with \texttt{use\_bn=False}, trained with SGD.
      \begin{solution}
        ...
        \end{center}
      \end{solution}
    \item (2 points) \textbf{(written)} Baseline CNN with \texttt{use\_bn=True}, trained with Adam.
      \begin{solution}
        ...
        \end{center}
      \end{solution}
    \item (4 points) \textbf{(written)} Shallow ResNet ($C=16$ with 1 and 3 blocks), trained with SGD.
      \begin{solution}
      ...
      \end{solution}
    \end{enumerate}
  \item (5 points) Performance of resnet3x16\_sgd on hidden testset. This is not reported,
